...
- Causeway's Wanderer now crosses grid steps smoothly, and the Play camera holds a small dead zone before easing after it. The generated scene also keeps its intended central start instead of a stale override placing it beside the world edge, so BUILD panning has terrain around it in every direction.
