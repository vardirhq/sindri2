...
- Weave `:hover` and `:active` now work in the running game: the editor's Play, native games and browser exports present the UI each frame with the pointer's state, and clicks land on the presented geometry, so a button a media query moved is clicked where it is drawn. Stylesheets can animate between states with CSS `transition` (durations, delays, `all`, and `ease`/`ease-in`/`ease-out`/`ease-in-out`/`linear`/`cubic-bezier` easings) on colours, lengths and numbers.
