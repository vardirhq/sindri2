...
//!
//! ```bash
//! cargo run -p orbital-baked --bin orbital-baked-capture -- out.png 390 844
//! ```
...
fn bind(
    ...
    let root = orbital_baked::project().join("assets");
...
async fn capture(
    ...
    // Bound after the run has played rather than before it, which is the whole
    // difference between this game and the one it was copied from. There the
    // enemies were vector shapes and the only textures were in the opening
    // scene, so binding up front saw everything there was to see. Here an enemy
    // is a baked sheet named by a prefab, and a prefab is a file the scene does
    // not mention: nothing spawned from one exists in the world until a script
    // spawns it. Binding first found no drifter, no asteroid and no boss, and
