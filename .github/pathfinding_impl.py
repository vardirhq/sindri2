...
def edit(path: str, old: str, new: str, count: int = 1) -> None:
    ...
    if text.count(old) < count:
        raise SystemExit(f"missing edit anchor in {path}: {old[:100]!r}")
    p.write_text(text.replace(old, new, count))


# Decay delegates pathfinding to the same validated world adapter as the engine.
edit(
    "crates/sindri-decay/Cargo.toml",
    'sindri-platform = { path = "../sindri-platform" }\n',
    'sindri-platform = { path = "../sindri-platform" }\n'
    'sindri-scene = { path = "../sindri-scene" }\n',
)

edit(
    "crates/sindri-decay/src/surface.rs",
...
edit(
    "crates/sindri-decay/src/scripts.rs",
    '''                    GridCall::Place => vec![
...
edit(
    "crates/sindri-decay/src/host.rs",
    "use sindri_grid::{GridPoint, GridSpace, PlanePoint, PlaneYAxis, Projection};\n",
    "use sindri_grid::{GridCoord, GridPathfinder, GridPoint, GridSpace, PlanePoint, PlaneYAxis, Projection};\n"
...
edit(
    ...
    '''    fn path_to_target(
        ...
        let target_world = self
            .transform_of(target)
            .ok_or_else(|| RuntimeError::Host(format!(
...
# Structural navigation data gets safe dedicated editor controls instead of
# generic opaque JSON rows.
edit(
    "editor/src/inspector.rs",
...
native = Path("editor/src/native.rs")
...
for old, new in replacements:
    if old not in text:
        raise SystemExit(f"missing native.rs anchor: {old[:100]!r}")
    text = text.replace(old, new, 1)

marker = "/// The rows of one payload, indented under its heading.\n"
if marker not in text:
    raise SystemExit("missing navigation section insertion marker")

navigation_ui = r'''fn grid_choices(world: &World) -> Vec<(String, String)> {
    world
        .entities()
        .filter_map(|(_, data)| {
            data.components.contains_key(tilemap::TYPE_NAME).then_some(())?;
...
fn grid_coord_row(ui: &mut egui::Ui, label: &str, value: &mut Value) -> bool {
    ...
    *value = serde_json::json!([
        numbers[0].round() as i64,
        numbers[1].round() as i64
    ]);
    true
}

fn grid_navigation_section(
    ui: &mut egui::Ui,
    payload: &mut Value,
    ...
    for (index, wall) in walls.iter_mut().enumerate() {
        ui.horizontal(|ui| {
            ui.add_space(10.0);
            ui.label(RichText::new(format!("Wall {}", index + 1)).size(10.0).color(TEXT_MUTED));
        ...
        let valid = first.zip(second).is_some_and(|(first, second)| {
            ...
            let (Some(ax), Some(ay), Some(bx), Some(by)) = (
                first[0].as_i64(), first[1].as_i64(), second[0].as_i64(), second[1].as_i64(),
            ) else {
                return false;
            };
    ...
    ui.horizontal(|ui| {
        ui.add_space(10.0);
        if ui.add_enabled(can_add, egui::Button::new("Add wall")).clicked() {
...
fn grid_occupant_section(
    ...
    for (index, cell) in footprint.iter_mut().enumerate() {
        ui.horizontal(|ui| {
            ui.add_space(10.0);
            ui.label(RichText::new(format!("Cell {}", index + 1)).size(10.0).color(TEXT_MUTED));
            ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                ui.add_space(7.0);
                if ui.add_enabled(may_remove, egui::Button::new("Remove")).clicked() {
                    remove = Some(index);
                }
            });
        });
...
'''
text = text.replace(marker, navigation_ui + marker, 1)

old = '''fn addable_components(
    components: &ComponentSchemaRegistry,
    present: &BTreeMap<String, Value>,
    first_font: Option<&str>,
    first_sprite: Option<&str>,
) -> Vec<ComponentMetadata> {'''
new = '''fn addable_components(
    components: &ComponentSchemaRegistry,
    present: &BTreeMap<String, Value>,
    first_font: Option<&str>,
    first_sprite: Option<&str>,
...
if old not in text:
    raise SystemExit("missing addable_components helper")
text = text.replace(old, new, 1)

old = '''        .filter(|metadata| !present.contains_key(&metadata.type_name))
        .filter(|metadata| {
            component_default(components, &metadata.type_name, first_font, first_sprite).is_some()
        })'''
new = '''        .filter(|metadata| !present.contains_key(&metadata.type_name))
        .filter(|metadata| {
...
if old not in text:
    raise SystemExit("missing addable filter")
text = text.replace(old, new, 1)

old = '''fn component_default(
    components: &ComponentSchemaRegistry,
    type_name: &str,
    first_font: Option<&str>,
    first_sprite: Option<&str>,
) -> Option<Value> {'''
new = '''fn component_default(
    components: &ComponentSchemaRegistry,
    type_name: &str,
    first_font: Option<&str>,
    first_sprite: Option<&str>,
...
if old not in text:
    raise SystemExit("missing component_default helper")
text = text.replace(old, new, 1)
text = text.replace(
    "    if type_name == TEXT_COMPONENT {",
    '''    if type_name == GRID_OCCUPANT_COMPONENT {
        return first_grid.map(|grid| serde_json::json!({
            "grid": grid,
            "footprint": [[0, 0]]
        }));
    }
...
# Existing unit-test call sites gain the contextual grid argument.
text = text.replace(
...
text = text.replace(
    'component_default(components, TEXT_COMPONENT, Some("fonts/Inter.ttf"), None)',
    'component_default(components, TEXT_COMPONENT, Some("fonts/Inter.ttf"), None, None)',
)
text = text.replace(
    'component_default(components, animation::TYPE_NAME, None, Some("idle"))',
    'component_default(components, animation::TYPE_NAME, None, Some("idle"), None)',
)
native.write_text(text)

# Gather proves the path end to end with one pathfinding-driven entity.
edit(
    "game/src/lib.rs",
    '''    (
...
Path("game/assets/scripts/wisp.decay").write_text(
...
scene_path = Path("game/assets/gather.scene.json")
scene = json.loads(scene_path.read_text())
floor = next(entity for entity in scene["entities"] if entity["id"] == "floor")
floor["components"]["sindri.grid_navigation"] = {
    "walls": [
        {"first": [0, 0], "second": [1, 0]},
        {"first": [1, 1], "second": [2, 1]},
        {"first": [1, 2], "second": [2, 2]},
    ]
}
if not any(entity["id"] == "wisp" for entity in scene["entities"]):
    scene["entities"].append(
        {
            "id": "wisp",
            "name": "Wisp",
            "transform_3d": {
                "position": [0.0, 1.65, 0.1],
            ...
            "components": {
                ...
                "sindri.script": {
                    "properties": {"step_seconds": 0.32},
                    "script": "Wisp",
                    "source": "scripts/wisp.decay",
                },
                "sindri.sprite": {
                    "layer": 15,
                    "space": "world",
                    "texture": "textures/orb.png",
                    "tint": [0.45, 0.82, 1.0, 0.72],
                },
            },
        }
    )
...
# Keep documentation contracts truthful.
scripting = Path("docs/scripting.md")
text = scripting.read_text()
if "Grid.step_toward" not in text:
...
if "typed Decay pathfinding" not in text and "## [Unreleased]\n" in text:
    text = text.replace(
        "## [Unreleased]\n",
...
matrix = Path("docs/feature-integration-matrix.md")
text = matrix.read_text()
text = text.replace(
    "| Pathfinding and occupancy | **Ready** — deterministic walls, A*, whole-footprint paths, authored components, and a validated world adapter | **Missing** | **Missing** | **Missing** | Add the editor adapter, then typed Decay access and a Gather use case |",
