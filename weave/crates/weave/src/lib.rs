...
fn parse_rules(source: String, inherited: Option<MediaCondition>) -> Result<Stylesheet, ParseError> {
    ...
    while !rest.trim_start().is_empty() {
        ...
        if let Some(after_media) = rest.strip_prefix("@media") {
            ...
            let condition = parse_media(query)?;
            let nested = parse_rules(body.to_owned(), Some(condition))?;
...
mod tests {
    ...
    #[test]
    fn parses_id_type_and_media_rules() {
        let sheet = parse(
            r#"
            #menu { width: 420px; direction: column; }
            sindri.ui.button { height: 56px; }
            @media (max-width: 700px) { #menu { width: 90vw; } }
            "#,
        )
        .expect("stylesheet parses");
        assert_eq!(sheet.rules.len(), 3);
        assert_eq!(sheet.rules[0].selector, Selector::Id("menu".into()));
        assert_eq!(sheet.rules[2].condition, Some(MediaCondition::MaxWidth(700.0)));
        assert!(sheet.rules[2].applies("menu", &[], Viewport { width: 390.0, height: 844.0 }));
        assert!(!sheet.rules[2].applies("menu", &[], Viewport { width: 900.0, height: 600.0 }));
    }
}
