...
impl<'a, H: Host> Runtime<'a, H> {
    ...
    pub fn call(
        ...
        let container = self
            ...
            .ok_or_else(|| RuntimeError::ContainerNotFound(container_name.to_owned()))?;

        let mut fields = self.initialize_fields(container)?;
        self.call_in_container(container, &mut fields, function_name, args)
    }
            ...
            match &instructions[ip] {
                ...
                Instruction::Call {
                    ...
                    let result = if callee.0.len() == 1
                        && container.functions.iter().any(|function| function.name == callee.0[0])
                    {
