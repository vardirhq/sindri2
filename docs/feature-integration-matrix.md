...
| Grid and isometric gameplay | **Partial** — typed geometry plus world/screen Y orientation; tilemaps share it for rendering and picking | **Partial** — tile painting uses the shared map adapter; no general grid/footprint/height tools | **Missing** — no grid value or host API | **Missing** — floor uses the grid module, but gameplay coordinates remain top-down | Add a typed Decay grid-position surface, then move Gather's gameplay onto it |
| Pathfinding and occupancy | **Missing** | **Missing** | **Missing** | **Missing** | Add footprints/occupancy, then renderer-free A* and script access |
