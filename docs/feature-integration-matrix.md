...
| Grid and isometric gameplay | **Partial** — renderer-free typed coordinates, bounds, neighbours, and orthogonal/isometric projection | **Missing** — no grid/footprint/height tools | **Missing** — no grid value or host API | **Missing** — floor looks isometric but gameplay coordinates are top-down | Add world/screen adapters, then move Gather onto the module |
