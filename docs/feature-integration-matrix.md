...
| Grid and isometric gameplay | **Partial** — typed geometry, footprints, occupancy, wall edges, pathfinding, and world/screen Y orientation; tilemaps share projection for rendering and picking | **Partial** — tile painting uses the shared map adapter; no footprint, occupancy, wall, or height tools | **Ready** — typed continuous position/read/place API through an explicit tilemap entity | **Ready** — movement, bounds, and collection use the floor's logical isometric coordinates | Adapt footprints, occupancy, and walls into engine/editor authoring |
| Pathfinding and occupancy | **Partial** — deterministic renderer-free walls, A*, and whole-footprint occupancy paths exist; no world adapter | **Missing** | **Missing** | **Missing** | Add engine/editor adapters, then typed Decay access and a Gather use case |
