...
| Decay scripts and exports | **Ready** — typed host, compile/run/reload, safe entity references, prefab references gathered from declared export types | **Partial** — component and `@export` fields; source cannot be created or edited, and a `Prefab` field has no picker | **Partial** — useful gameplay subset; no collections, LSP, formatter, or debugger | **Ready** — all gameplay rules are Decay | Bounded queries and a collection value, then source tooling |
