...
| Persistent voxel compiled/GPU mesh cache | 🟡 | ❌ | ❌ | ❌ | **Behind** | `SectionMeshCache<T>` keys compiled representations by section and block/smooth/hybrid/custom profile, retains old geometry during replacement, rejects stale results, and can release all profiles named by a leaving residency delta. `sindri-render` now likewise caches textured GPU buffers by opaque identity and monotonic revision, keeps old buffers drawable until replacement upload, supports 32-bit section indices, and exposes reuse/upload/release counters. The voxel material/identity bridge and Voxel Lab proof remain. |
