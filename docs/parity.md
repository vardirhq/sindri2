...
| Stackable tile volumes | ✅ | 🟡 | ❌ | ✅ | **Behind** | Sparse XYZ storage, tilesets, face culling/rendering, Gather proof, and undoable surface/level block painting work. A tile also declares how much of its cell it fills, so slabs and other partial heights are cells rather than art tricks, and occlusion asks whether a neighbour covers a face *completely*. Orthogonal and isometric volumes both work: the projection decides which faces a view can see and what counts as behind, and a cell's depth comes from its column rather than from each drawn face, so a stack does not walk toward the viewer as it grows. Navigation reads the volume too: a column's surface is the top of its highest solid cell, a column with nothing solid in it is a hole rather than ground, and `max_step` decides which edges between columns are walls — so a slab is a step where a block is a wall. Side picking, terrain rules, physics collision, asymmetric climb and drop, overhangs as second walkable levels, and slice views remain |
