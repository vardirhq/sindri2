...
| Voxel dirty-region tracking | 🟡 | ❌ | ❌ | ❌ | **Behind** | A voxel edit dirties its section plus a neighbouring section when the edit touches that boundary, and queues resident affected sections for remeshing. There is no persistent compiled/GPU section cache yet. |
| Neighbour-aware block meshing | 🟡 | ❌ | ❌ | ❌ | **Behind** | Engine block mesher emits only exposed semantic faces and samples through `VoxelSource` across section boundaries, so internal faces between adjacent solid sections disappear without neighbour residency. Render classes, material mapping, compiled vertices/indices, and Causeway proof remain. |
