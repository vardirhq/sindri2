...
| Stackable tile volumes | ✅ | 🟡 | ❌ | ✅ | **Behind** | Sparse XYZ storage, tilesets, face culling/rendering, Gather proof, and undoable surface/level block painting work. A tile also declares how much of its cell it fills, so slabs and other partial heights are cells rather than art tricks, and occlusion asks whether a neighbour covers a face *completely*. Side picking, terrain rules, collision/navigation, and slice views remain |
