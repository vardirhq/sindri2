...
| Stackable tile volumes | ✅ | 🟡 | 🟡 | ✅ | **Behind** | Sparse XYZ storage, tile sets, face culling and rendering, and undoable surface/level block painting work. A tile declares how much of its cell it fills, so slabs are cells rather than art tricks, and occlusion asks whether a neighbour covers a face *completely*. Both projections work: the projection decides which faces a view can see and what counts as behind, and a cell's depth comes from its column rather than from each drawn face. A tile says whether its top holds anything up and, separately, whether a walker can stand on it: water supports and is unwalkable, so a pond is no longer indistinguishable from a hole, and something can rest on one. Navigation reads the volume — a column's walkable surface is the top of its highest walkable cell, a column with nothing walkable in it is unwalkable whether it is water or empty, and `max_step` decides which edges are walls, so a slab is a step where a block is a wall. Decay reads and writes a cell with `Grid.block`/`Grid.set_block`, and asks whether a walker can stand at a point with `Grid.walkable` — the same walkable surface the pathfinder reads, so a script and a wisp cannot disagree about where the ground is. **Gather's player consults it**: before, it compared positions against tagged props, which knows nothing about terrain, so the moat was walked across and the outcrop walked into while the Wisp routed around both. A script can ask whether a cell can be *stood on* but not yet whether it can be *stepped into from where it is*, so a script-driven walker does not honour `max_step` the way navigation does. `layer_step` spreads a volume's cells across render layers, which is what lets blocks interleave with sprites in a 2D scene. **Gather's whole floor is a volume**: 625 blocks and no `sindri.tilemap` anywhere in the project. A tile may declare several looks with weights, chosen per cell from a stable in-repo hash of the cell, the tile's name and the volume's `variant_seed`, so variation costs nothing per cell and reproduces exactly — but **Gather still hand-scatters five kinds of grass across 252 cells**, because collapsing them into one tile with variants preserves the proportions and rearranges which cell gets which, and that is a change to authored art rather than a migration. A volume is resolved into its faces once and replayed, with only each cell's depth re-measured against the camera, which took extracting Gather's farm from 6.7 ms a frame to 1.2 ms. Side picking, terrain rules, physics collision, asymmetric climb and drop, overhangs as second walkable levels, and slice views remain |
