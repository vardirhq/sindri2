...
| Persistent voxel compiled/GPU mesh cache | 🟡 | ❌ | ❌ | ❌ | **Behind** | `SectionMeshCache<T>` keys compiled representations by section and block/smooth/hybrid/custom profile, retains old geometry during replacement, rejects stale results, and can release all profiles named by a leaving residency delta. The scene/render bridge does not yet store GPU buffers in it, and Causeway still uses its tile-volume path. |
