...
| **Tilemap** | `render/tilemap.rs` (135) | **Port** | Small and clean: `Tile`, `set_tile`, `fill_rect`, `tile_to_world`, `world_to_tile`, `tile_uv_rect`. Legacy models it as a render type; Sindri Next needs it as a schema'd component so the editor can author it and the scene can carry it. Same maths, different home. Depended on the same UV rect work as animation, which now exists. |
