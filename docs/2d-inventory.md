...
| **Tilemap** | `render/tilemap.rs` (135) | **Ported** | Small and clean: `Tile`, `set_tile`, `fill_rect`, `tile_to_world`, `world_to_tile`, `tile_uv_rect`. Legacy models it as a render type; Sindri Next needs it as a schema'd component so the editor can author it and the scene can carry it. Same maths, different home. Depended on the same UV rect work as animation, which now exists. Done: `sindri.tilemap` carries the sheet grid, the map grid, and a flat array of cells, and extracts into the same sprite batches loose sprites use. `tile_to_world`/`world_to_tile` became `tile_to_local`/`local_to_tile`, on the component, because the map's origin is its entity's transform. It gained a projection the legacy type did not have, because the first thing to use it was an isometric floor. |
