...
| --- | --- | --- | --- |
| **Sprite animation and sheets** | `render/animation.rs` (160) | **Refactored** | The clip model — frames, durations, looping, `from_grid` — ports nearly as-is. But a legacy `AnimationFrame` holds a `TextureHandle` per frame, which under Sindri Next's registry would mean one GPU texture per frame of every animation. Sindri Next instead names UV rects in one sheet: `sindri.animation.sprite` stores clips of those names, while the cursor stays outside the world so playing or previewing a clip does not dirty the scene. The editor slices and names the sheet, authors clip order and timing, and previews the result against the real texture. |
...
| **Text rendering** | `render/text.rs` (95), `fonts.rs` | **Replaced** | Glyphon remains the shaping and wgpu renderer, but the manual font path is gone: `FontAssetDecoder` validates project-owned bytes, `sindri.ui.text` extracts an anchored overlay command, and every host binds the same logical font reference. The editor adds text from an available project font, edits multiline content, and picks the font asset. |
...
| **HUD** | `hud.rs` (382) | **Defer** | Anchored screen text and images. Both now exist as the `sindri.ui.*` component family, so a HUD is composition rather than a subsystem — Gather's banner, score lamps, and title are ordinary UI entities. Revisit if layout beyond anchor plus offset is wanted. |
